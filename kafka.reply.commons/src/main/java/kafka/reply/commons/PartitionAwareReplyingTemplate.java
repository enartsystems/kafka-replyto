 ...
 */
@Slf4j
public class PartitionAwareReplyingTemplate<K, V, R> extends ReplyingKafkaTemplate<K, V, R> implements PartitionAwareReplyingOperations<K, V, R> {

  public PartitionAwareReplyingTemplate(ProducerFactory<K, V> producerFactory,
  ...
  private TopicPartition getFirstAssignedReplyTopicPartition() {
    if (getAssignedReplyTopicPartitions() != null &&
      ...
      if (log.isDebugEnabled()) {
        this.log.debug("Using partition " + replyPartition.partition());
      }
